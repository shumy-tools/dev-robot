...
class MachineTest {
  @Test fun testFirstState() {
    ...
    assert(server.query(MEntity::class,"{ *, @history { evt, from, to } }").toString() == "[{@id=$id, @state=START, name=My Name, @history=[{@id=1, evt=null, from=null, to=START}]}]")
  ...
  @Test fun testAction() {
    ...
    eInst1.all[0].checkHistory("dr.test.MEntityMachine.Event.Submit","""{"value":"#try-submit"}""","START","VALIDATE","""{"owner":"shumy"}""")
    ...
    assert(server.query(MEntity::class,"| @id == 1 | { @state, @history { evt, from, to, data } }").toString() == """[{@id=1, @state=VALIDATE, @history=[{@id=1, evt=null, from=null, to=START, data={"d-field":30}}, {@id=2, evt={"value":"#try-submit"}, from=START, to=VALIDATE, data={"owner":"shumy"}}]}]""")
    ...
    assert(server.query(MEntity::class,"| @id == 1 | { @state, @history { evt, from, to, data } }").toString() == """[{@id=1, @state=STOP, @history=[{@id=1, evt=null, from=null, to=START, data={"d-field":30}}, {@id=2, evt={"value":"#try-submit"}, from=START, to=VALIDATE, data={"owner":"shumy"}}, {@id=3, evt={}, from=VALIDATE, to=STOP, data={}}]}]""")
